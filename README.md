# 🧮 Portafolio de Matemáticas Discretas

¡Bienvenido/a a mi portafolio digital! En este repositorio se encuentra recopilado todo el desarrollo, actividades prácticas y aprendizaje continuo de la asignatura de **Matemáticas Discretas**. 

Para reflejar mi evolución y el uso de diferentes herramientas tecnológicas, el portafolio está dividido en dos grandes bloques: la fundamentación teórica y práctica (Unidades 1 y 2) alojada en **Google Drive**, y el desarrollo avanzado (Unidad 3) trabajado directamente aquí en **GitHub**.

---

## 📂 Unidad 1: Lógica Matemática (Google Drive)
En esta unidad me enfoqué en desarrollar habilidades de razonamiento lógico para solucionar problemas complejos, comprendiendo la estructura formal del pensamiento matemático.

* **Fases Desarrolladas:**
  * **Fase 1:** Proposiciones lógicas y conectores fundamentales.
  * **Fase 2:** Construcción y operación de Tablas de Verdad.
  * **Fase 3:** Equivalencias lógicas y Formas Normales.
  * **Fase 4 y 5:** Inferencia lógica, razonamiento deductivo, tautologías y argumentos válidos.
* **Actividades incluidas:** Ejercicios prácticos colaborativos (APE), lecturas autónomas y resoluciones de problemas de deducción.

[▲ Acceder a la Carpeta de la Unidad 1 en Google Drive](https://drive.google.com/drive/folders/1Pb7JutDFCcGrppZKkuXkWPoJGhB5YY-S?usp=drive_link)

---

## 📂 Unidad 2: [Nombre de tu Unidad 2] (Google Drive)
*Nota: Recuerda cambiar el título de arriba con el nombre real de tu Unidad 2.*

En la segunda unidad se dio continuidad al programa mediante el estudio profundo de [menciona brevemente de qué trató, por ejemplo: Teoría de Conjuntos / Relaciones / Conteo].

* **Temas Clave del Portafolio:**
  * [Tema importante 1, ej: Operaciones de conjuntos]
  * [Tema importante 2, ej: Propiedades de las relaciones]
  * [Tema importante 3, ej: Aplicaciones prácticas]
* **Actividades incluidas:** Guías de práctica experimental, problemas resueltos y material de aprendizaje autónomo.

[▲ Acceder a la Carpeta de la Unidad 2 en Google Drive](AQUÍ_PEGA_EL_LINK_DE_TU_DRIVE_UNIDAD_2)

---

## 💻 Unidad 3: [Nombre de tu Unidad 3] (Desarrollo en GitHub)
A partir de esta unidad, el portafolio se traslada completamente a GitHub para integrar conceptos matemáticos con el control de versiones y documentación en código.

*(Próximamente se listarán aquí los temas, códigos y laboratorios de la Unidad 3 a medida que los vayamos estructurando).*
